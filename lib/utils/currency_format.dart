...
stringToCurrency(double idr, context) {
  ...
  symbol = currencySetting.currency.description != null ? convertHtmlUnescape(currencySetting.currency.description) : '';
  code = currencySetting.currency.title;
  decimalNumber = currencySetting.formatCurrency.slug != null ? int.parse(currencySetting.formatCurrency.slug) : 0;
  thousandSeparator = currencySetting.formatCurrency.image ?? ".";
  decimalSeparator = currencySetting.formatCurrency.title ?? ",";
  thousandSeparator == '.' && decimalSeparator == '.' ? decimalSeparator = ',' : decimalSeparator = '.';
  var pattern = '';
  if (decimalNumber == 0) {
  ...
  if (thousandSeparator == '.' && decimalSeparator == ',') invertSeparators = true;
  final currency = Currency.create(code, 3, invertSeparators: invertSeparators, symbol: symbol, pattern: pattern);
