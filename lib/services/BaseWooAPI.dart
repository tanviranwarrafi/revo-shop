...
class BaseWooAPI {
  ...
  _getOAuthURL(String requestMethod, String endpoint, version, bool isCustom, bool isOrder) {
  ...
  Future<dynamic> getAsync(String endPoint, {int version = 3, bool isCustom = false, bool isOrder = false, bool printedLog = false}) async {
    String url = this._getOAuthURL("GET", endPoint, version, isCustom, isOrder);
    var response;

    DateTime start = DateTime.now();
    if (printedLog) printLog("[api][${DateTime.now().toString().split(' ').last}] getAsync START [endPoint:$endPoint]");
    ...
    if (printedLog) printLog("Result : ${response.body}");

    return response;
  }

  Future<dynamic> postAsync(String endPoint, Map data,
      {int version = 3, bool isCustom = false, bool isOrder = false, bool printedLog = false}) async {
    var url = this._getOAuthURL("POST", endPoint, version, isCustom, isOrder);

    DateTime start = DateTime.now();

    printLog("[api][${DateTime.now().toString().split(' ').last}] postAsync START [endPoint:$endPoint] url:$url");
  ...
  Future<dynamic> putAsync(String endPoint, Map data, {int version = 3, bool isCustom = false, bool isOrder = false}) async {
