...
class OrderProvider with ChangeNotifier {
  ...
  Future checkOutOrder(context, {int totalSelected, List<ProductModel> productCart, Future<dynamic> Function() removeOrderedItems}) async {
      ...
      if (Session.data.getBool('isLogin')) {
        ...
        productCart.forEach((element) {
          if (element.isSelected) {
            ...
            cart.listItem.add(
              CartProductItem(
                  productId: element.id, quantity: element.cartQuantity, variationId: element.variantId, variation: [variation]),
            );
          }
        });
        ...
        cart.listCoupon = [];
        //check coupon
        if (coupons.couponUsed != null) cart.listCoupon.add(new CartCoupon(code: coupons.couponUsed.code));
        //add to cart model
        ...
        await Provider.of<OrderProvider>(context, listen: false).checkout(order).then((value) async {
          printLog(value, name: 'Link Order');
          await Navigator.push(context, MaterialPageRoute(builder: (context) => CheckoutWebView(url: value, onFinish: removeOrderedItems)));
        });
      } else {
        snackBar(context, message: AppLocalizations.of(context).translate('you_login_first'));
      }
    }
  }

  Future buyNow(context, ProductModel product, Future<dynamic> Function() onFinishBuyNow) async {
    if (Session.data.getBool('isLogin')) {
      CartModel cart = new CartModel();
      cart.listItem = [];
      cart.listItem.add(new CartProductItem(productId: product.id, quantity: product.cartQuantity, variationId: product.variantId));
  ...
  Future loadItemOrder(context) async {
    ...
    if (detailOrder != null) {
      ...
      detailOrder.productItems.forEach((element) async {
        await Provider.of<ProductProvider>(context, listen: false).fetchProductDetail(element.productId.toString()).then((value) {
  ...
  Future<void> actionBuyAgain(context) async {
    detailOrder.productItems.forEach((elementOrder) {
      listProductOrder.forEach((element) {
        if (element.id == elementOrder.productId) {
          ...
          element.priceTotal = double.parse(element.productPrice) * element.cartQuantity;
    ...
    snackBar(context, message: AppLocalizations.of(context).translate('add_cart_message'));
  }
  ...
  Future addCart(ProductModel product, context) async {
    ...
    if (!Session.data.containsKey('cart')) {
      ...
      List<ProductModel> listCart = products.map((product) => ProductModel.fromJson(product)).toList();

      printLog(listCart.toString(), name: 'List Cart');

      int index = products.indexWhere((prod) => prod["id"] == product.id && prod["variant_id"] == product.variantId);

      if (index != -1) {
        product.cartQuantity = listCart[index].cartQuantity + product.cartQuantity;
