      ...
      if (initialLink != null) {
        ...
        if (uri.pathSegments[0] == "shop" && uri.pathSegments[1].isNotEmpty) {
          ...
          onLinkClicked = () async => await Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => ProductDetail(slug: uri.pathSegments[1])),
              );
        }
        /*Blog (Detail Blog)*/
        if (uri.pathSegments[0] == "artikel" && uri.pathSegments[1].isNotEmpty) {
          print("Detail Blog");
          onLinkClicked =
              () async => await Navigator.push(context, MaterialPageRoute(builder: (_) => BlogDetail(slug: uri.pathSegments[1])));
        }
        screen = SplashScreen(onLinkClicked: onLinkClicked);
      }
